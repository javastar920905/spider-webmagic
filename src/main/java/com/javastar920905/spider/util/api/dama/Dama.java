...
public class Dama {
  ...
  // uname：用户名
  // upwd：用户密码
  public Dama(int appID, String softKey, String uname, String upwd) {
    _appID = appID;
    _softKey = softKey;
    _uname = uname;
    _upwd = upwd;
  ...
  public RequestResult register(String qq, String email, String tel) {
    ...
    url = url + "?appID=" + _appID + "&qq=" + qq + "&email=" + email + "&tel=" + tel + "&encinfo="
        + encInfo;
    ...
    RequestResult result = handleRequest(new IActualHandler() {
      ...
      public RequestResult process() {
        ...
        return doRequest(url, new IResultHandler() {
          @Override
          public RequestResult handleSucc(Map<String, String> m) {
            return new ReadInfoResult(0, "", m.get("name"), m.get("qq"), m.get("email"),
                m.get("tel"));
  ...
  public RequestResult decodeUrl(final String urlPic, final String cookie, final String referer,
      final int type, final int timeout) {
  ...
  // timeout：超时秒数
  // RequestResult.ret < 0 failed, ret is error code; > 0, ret is id
  public DecodeResult decodeUrlAndGetResult(String urlPic, String cookie, String referer, int type,
      int timeout) {
  ...
  private RequestResult decode(final int type, final int timeout, final byte[] data,
      final String txt) {
    return handleRequest(new IActualHandler() {
      @Override
      public RequestResult process() {
  ...
  private RequestResult doRequest(String url, IResultHandler handler, boolean saveAuth,
      boolean loadAuth) {
  ...
  private RequestResult doRequest(String url, IResultHandler handler,
      IPostDataGettor postDataGettor, boolean saveAuth, boolean loadAuth) {
    try {
      // 在URL地址中附带授权信息
      String auth = null;
      if (loadAuth) {
  ...
  // 登陆
  private RequestResult login(String preAuthInfo, int appID, String softKey, String uname,
      String upwd) {
